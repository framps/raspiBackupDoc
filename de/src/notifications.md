...
1) eMail - [Beispielkonfigurationen](msmtp-configuration-for-web-de-account.md)
2) Pushover - [Konfigurationsoptionen](general-config-options.md)
3) Telegram - [Konfigurationsoptionen](general-config-options.md)
4) Slack - [Konfigurationsoptionen](general-config-options.md)
5) Jeder beliebige andere Dienst vermittels der eMailErweiterung oder der Benachrichtigungserweiterung

Die hauptsächlichen Optionen für die Benachrichtigungen sind [hier](general-config-options.md) beschrieben. Die eMail Erweiterung sowie die Benachrichtigungserweiterung ist [hier](hooks-for-own-scripts.md) beschrieben.
...
> Will man die Benachrichtigung sowie dessen Einstellungen testen, kann die Option -F genutzt werden, die ein Backuplauf nur simuliert, also kein Backup erstellt wird,sowie Benachrichtigungen geschickt werden.
